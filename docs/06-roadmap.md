...
### 1a. 能跑起来（开源落地基线）
- [ ] 项目脚手架（前端 React/TS + 后端 FastAPI + 适配器接口骨架）
- [ ] `.env.example` + README + docker-compose 一键启动（含可选 Ollama）
- [ ] 首次配置向导：选存储 → 配模型 provider/key → 测连通性 → 水平基线测试
- [ ] 存储层：LocalAdapter（SQLite）先行
- [ ] LLMProvider：OpenAICompatAdapter + ClaudeAdapter

### 1b. 核心闭环
- [ ] 功能1：切词 + 逐词问询 + 生词入库（含来源句）
- [ ] 功能3a：理解式背单词（基于生词本）
- [ ] FSRS 调度器接入
- [ ] 功能2c：自由写作 + 延迟纠错 + 雅思/托福打分
- [ ] ErrorAnalysisAgent + 错题本
- [ ] 水平基线分级（首次使用，并入配置向导）
